...
def _get_local_files(local_dir, pattern=''):
    ...
    if pattern:
        ...
        local_list = set([])
        for p in patterns:
            local_list = local_list | set(glob(p))
        for path in local_list:
            directory, filename = os.path.split(path)
            if os.path.isfile(path):
                local_files[directory] = local_files.get(directory, []) + \
                    [filename]
            elif os.path.isdir(path):
                local_files[filename] = local_files.get(directory, [])
        os.chdir(cwd)
    return local_files


def _stage_local_files(local_dir, local_files=None):
    """
    ...
    """
    # Set local_files to empty dict if no value supplied.
    ...
    for root, _, files in os.walk(local_dir):
        ...
        for filename in files:
            if relative_tree:
                filepath = os.path.join(relative_tree, filename)
                if not os.path.exists(
                        os.path.join(staging_dir, relative_tree)):
                    os.mkdir(os.path.join(staging_dir, relative_tree))
            else:
                filepath = filename
            shutil.copy2(os.path.join(root, filename),
                    os.path.join(staging_dir, filepath))
    return staging_dir


def deploy_files(local_dir, remote_dir, pattern='',
        rsync_exclude=None,
        use_sudo=False):
    """
    Generic deploy function for cases where one or more files are being
    deployed to a host. Wraps around ``rsync_project`` and stages files
    locally and/or remotely for network efficiency.
    ...
    Note: Unlike other ways of deploying files, all files under
    local_dir will be deployed into remote_dir. This is the equivalent
    to cp -R local_dir/* remote_dir.

    ``pattern`` finds all the pathnames matching a specified glob
    pattern relative to the local_dir according to the rules used by the
    Unix shell.  ``pattern`` enhances the basic functionality by
    allowing the python | to include multiple patterns. eg
    '*.txt|Django*'
    ...
    """
    # Set rsync_exclude to default array if no value supplied.
    ...
    # If we are only copying specific files or rendering templates we
    # need to stage locally.
    if local_files:
        staging_dir = _stage_local_files(local_dir, local_files)
    ...
    rsync_project(local_dir=staging_dir,
            remote_dir=remote_staging_dir,
            exclude=rsync_exclude,
            delete=True)

    # Create the final destination.
    mkdirs(remote_dir, use_sudo)

    if not os.listdir(staging_dir):
        return created_list

    func = use_sudo and sudo or run
    # Copy recursively from the staging to the destination and keep a
    # list.
    remote_base_path = '/'.join([
            remote_staging_dir,
            os.path.basename(local_dir),
            '*',
            ])
    copy_file_list = func(' '.join(
            ['cp -Ruv', remote_base_path, remote_dir])).split('\n')
    if copy_file_list[0]:
        created_list += [filename.split(' ')[2][1:-1]
            for filename in copy_file_list if filename]
...
def mkdirs(remote_dir, use_sudo=False):
    ...
    # Extract dir list from [
    #   "mkdir: created directory
    #   'example.com/some/dir'"].
    if result[0]:
        result = [directory.split(' ')[3][1:-1]
            for directory in result if directory]
    return result


def upload_template(filename, destination,
        context=None,
        use_sudo=False,
        backup=True,
        modified_only=False):
    """
    Render and upload a template text file to a remote host using the
    Django template api.
    ...
    """
    # Set context to empty dict if nothing supplied.
    if context is None:
        context = {}
    ...
    if modified_only:
        ...
        if hashfile:
            if not exists(hashfile_dir):
                sudo('mkdir -p %s' % hashfile_dir)
            # Store the hash near the template.
            sudo('touch %s' % hashfile_path)
            previous_hashed = sudo('cat %s' % hashfile_path).strip()
            if previous_hashed == hashed:
                return False
            else:
                sudo('echo %s > %s' % (hashed, hashfile_path))

    temp_destination = '/tmp/' + basename

    # This temporary file should not be automatically deleted on close,
    # as we need it there to upload it (Windows locks the file for
    # reading while open).
    ...
    # Back up any original file (need to do figure out ultimate
    # destination).
    if backup:
