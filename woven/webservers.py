...
from woven.deployment import (mkdirs, upload_template)
...
def _ls_sites(path):
    """
    List only sites in the domain_sites() to ensure we co-exist with
    other projects.
    """
    with cd(path):
        sites = run('ls').split('\n')
...
def _sitesettings_files():
    ...
    settings.py can be prefixed with a subdomain and underscore so with
    example.com site:
    sitesettings/settings.py would be the example.com settings file and
    sitesettings/admin_settings.py would be the admin.example.com
    settings file.
    """
    settings_files = []
    ...
    if os.path.exists(sitesettings_path):
        ...
        for filename in sitesettings:
            if filename == 'settings.py':
                settings_files.append(filename)
            elif len(filename) > 12 and filename[-12:] == '_settings.py':
                # Prefixed settings.
                settings_files.append(filename)
...
def _get_django_sites():
    ...
    if (not env.sites and
            'django.contrib.sites' in env.INSTALLED_APPS and
            deployed):
        with cd('/'.join([
                deployment_root(),
                'env',
            ...
            venv = '/'.join([
                    ...
                    'bin',
                    'activate'])
            # Since this is the first time we run ./manage.py on the
            # server, it can be a point of failure for installations.
            with settings(warn_only=True):
                output = run(' '.join(['source', venv, '&&',
                        "django-admin.py dumpdata sites " \
                            "--settings=%s.sitesettings.settings" % \
                            env.project_package_name]))

                if output.failed:
                    print "ERROR: There was an error running ./manage.py " \
                        "on the node"
                    print "See the troubleshooting docs for hints on how " \
...
def domain_sites():
    ...
    if not hasattr(env, 'domains'):
        ...
        for site_id in site_ids:
            for filename in _sitesettings_files():
                domain = _AttributeDict({})

                if filename == 'settings.py':
                    domain.name = sites[site_id]
                else:
                    # Prefix indicates subdomain.
                    subdomain = filename[:-12].replace('_', '.')
                    domain.name = ''.join([subdomain, sites[site_id]])

                domain.settings = filename
                domain.site_id = site_id
                domains.append(domain)

        env.domains = domains
        if env.domains:
            env.root_domain = env.domains[0].name
        else:
            domain.name = _root_domain()
            domain.site_id = 1
            domain.settings = 'settings.py'
            env.domains = [domain]
...
def deploy_webconf():
    ...
    # TODO - incorrect - check for actual package to confirm
    # installation.
    if webserver_list():
        if env.verbosity:
...
def deploy_wsgi():
    ...
    # Ensure project apps path is also added to environment variables as
    # well as WSGI.
    ...
    for settings_file in _sitesettings_files():
        ...
        with cd(remote_dir):
            settings_module = settings_file.replace('.py', '')
            context = {
                "deployment_root": deployment_root(),
                "user": env.user,
                "project_name": env.project_name,
            ...
            if wsgi == 'apache2':
                filename = settings_file.replace('.py', '.wsgi')
                upload_template(
            ...
            if env.verbosity:
                print " * uploaded", filename
            # Finally set the ownership/permissions. We'll use the
            # group to allow www-data execute.
            if wsgi == 'apache2':
