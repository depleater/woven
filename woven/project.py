...
def _make_local_sitesettings(overwrite=False):
    local_settings_dir = os.path.join(
            os.getcwd(), env.project_package_name, 'sitesettings')
    ...
    if not os.path.exists(settings_file_path):
        ...
        manage_path = os.path.join(
                os.getcwd(), env.project_package_name, 'manage.py')
        dest_manage_path = os.path.join(
                os.getcwd(),
...
def deploy_project():
    ...
    # Exclude a few things that we don't want deployed as part of the
    # project folder
    rsync_exclude = [
        'local_settings*',
        '*.pyc',
    ...
    created = deploy_files(local_dir, project_root,
            rsync_exclude=rsync_exclude)
    if not env.patch:
        # hook the project into sys.path
        pyvers = run('python -V').split(' ')[1].split('.')[0:2]  # Python x.x.x
        sitepackages = ''.join(['lib/python', pyvers[0], '.', pyvers[1],
                '/site-packages'])
        link_name = '/'.join([deployment_root(), 'env',
                env.project_fullname,
                sitepackages,
                env.project_package_name])
...
def deploy_static():
    ...
    if not env.STATIC_URL or 'http://' in env.STATIC_URL:
        return
    ...
    # If app media is not handled by django-staticfiles we can install
    # admin media by default.
    if ('django.contrib.admin' in env.INSTALLED_APPS and
            not 'django.contrib.staticfiles' in env.INSTALLED_APPS):
        if (env.MEDIA_URL and
                env.MEDIA_URL == env.ADMIN_MEDIA_PREFIX[:m_prefix]):
            print "ERROR: Your ADMIN_MEDIA_PREFIX (Application media) " \
                "must not be on the same path as your MEDIA_URL (User media)"
            sys.exit(1)
        ...
        if env.MEDIA_URL and env.MEDIA_URL == env.STATIC_URL[:m_prefix]:
            print "ERROR: Your STATIC_URL (Application media) must not " \
                "be on the same path as your MEDIA_URL (User media)"
            sys.exit(1)
    ...
    if not rollback:
        ...
        if env.DEFAULT_DATABASE_ENGINE == 'django.db.backends.sqlite3':
            ...
            if env.DEFAULT_DATABASE_NAME[0] not in [os.path.sep, '.']:
                ...
                print "ERROR: Use a full expanded path to the " \
                    "database in your Django settings"
                sys.exit(1)
            else:
            ...
            if not db_path or not os.path.exists(db_path):
                print "ERROR: the database %s does not exist.\n" \
                    ...
                    "locally first, or check your settings." % db_path
                sys.exit(1)
            ...
            print "ERROR: The default database engine has not been defined " \
                "in your Django settings file"
            print "At a minimum you must define an sqlite3 database " \
                "for woven to deploy, or define a database that is " \
                "managed outside of woven."
            sys.exit(1)
    elif (rollback and
            env.DEFAULT_DATABASE_ENGINE == 'django.db.backends.sqlite3'):
        if env.INTERACTIVE:
