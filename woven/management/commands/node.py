...
class Command(WovenCommand):
    ...
    option_list = WovenCommand.option_list + (
        make_option('--options',
            help='Store all the management command options in a string. ' \
                'ie --options="--[opt]=[value] ..."'),
    )
    ...
    def parse_host_args(self, *args):
        """
        Splits out the management command and returns a comma
        separated list of host_strings.
        """
    ...
    def handle_host(self, *args, **kwargs):
        ...
        command = args[0]
        path = '/home/%s/%s/env/%s/project/%s/' % (
                env.user,
                root_domain(),
                env.project_fullname,
                env.project_name,
                )
        pythonpath = '/home/%s/%s/env/%s/bin/python' % (
                env.user,
                env.root_domain,
                env.project_fullname,
                )
