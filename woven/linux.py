...
"""
# To implement different backends we'll either split out functions
# into function and _backend_functions, or if the difference is
# marginal just use if statements.
...
from woven.deployment import (
        _backup_file,
        _restore_file,
        upload_template,
        )
from woven.environment import (
        server_state,
        set_server_state,
        get_packages,
        )
...
def add_repositories():
    ...
    """
    if (not env.overwrite and
            env.LINUX_PACKAGE_REPOSITORIES == server_state(
                    'linux_package_repositories')):
        return
    if env.verbosity:
        print env.host, "UNCOMMENTING SOURCES in " \
                "/etc/apt/sources.list and adding PPAs"
    if contains(
            filename='/etc/apt/sources.list',
            text='#(.?)deb(.*)http:(.*)universe'):
    ...
    set_server_state('linux_package_repositories',
            env.LINUX_PACKAGE_REPOSITORIES)
...
def disable_root():
    """
    Disables root and creates a new sudo user as specified by
    HOST_USER in your settings or your host_string.

    The normal pattern for hosting is to get a root account which is
    then disabled.
    ...
    def enter_password():
        password1 = getpass.getpass(
                prompt='Enter the password for %s:' % sudo_user)
    ...
    if env.verbosity and not (env.HOST_USER or env.ROLEDEFS):
        ...
        if env.INTERACTIVE:
            root_user = prompt("\nWhat is the default " \
                        "administrator account for your node?",
                    default=env.ROOT_USER)
        else:
        ...
        if env.INTERACTIVE:
            sudo_user = prompt(
                    "What is the new or existing account you wish to " \
                        "use to setup and deploy to your node?",
                    default=sudo_user)
    ...
    with settings(host_string=host_string,
        ...
        if not exists(home_path):
            if env.verbosity:
                print env.host, 'CREATING A NEW ACCOUNT " \
                        "WITH SUDO PRIVILEGE: %s' % sudo_user
            if not original_password:

                original_password = enter_password()

            add_user(username=sudo_user,
        ...
        if env.key_filename:
            ...
            sudo('cp -f ~/.ssh/authorized_keys ' \
                    '/home/%s/.ssh/authorized_keys' % sudo_user)
...
def install_packages():
    ...
    if (env.overwrite or not server_state('apparmor-disabled')
            and 'apparmor' in p):
    ...
    # Ubuntu packages.
    if ((env.overwrite or not server_state('pip-venv-wrapper-installed'))
            and 'python-setuptools' in packages):
    ...
    if not contains("/home/%s/.profile" % env.user,
            "source /usr/local/bin/virtualenvwrapper.sh"):
        append("/home/%s/.profile" % env.user, "export WORKON_HOME=$HOME/env")
        append("/home/%s/.profile" % env.user,
                "source /usr/local/bin/virtualenvwrapper.sh")
...
def port_is_open():
    ...
    with settings(hide('aborts'), warn_only=True):
        try:
            if env.verbosity:
                print "Testing node for previous installation on " \
                    "port %s:" % env.port
            ...
            # No way to catch the failing connection without catchall?
            return False
        if distribution.distributor_id != 'Ubuntu':
            print env.host, 'WARNING: Woven has only been tested on " \
                "Ubuntu >= 10.04. It may not work as expected on', \
...
def restrict_ssh(rollback=False):
    ...
    if not rollback:
        ...
        upload_template('woven/ssh/sshd_config',
                '/etc/ssh/sshd_config',
                context=context,
                use_sudo=True)
        # Restart sshd.
        ...
        proceed = True
        if (not env.key_filename and
                (env.DISABLE_SSH_PASSWORD or env.INTERACTIVE) and
                contains('/etc/ssh/sshd_config',
                        '#PasswordAuthentication no',
                        use_sudo=True)):
            print "WARNING: You may want to test your node ssh login " \
                "at this point ssh %s@%s -p%s" % (
                        env.user,
                        env.host,
                        env.port,
                        )
            c_text = 'Would you like to disable password login and use ' \
        ...
        if not env.INTERACTIVE or proceed or env.DISABLE_SSH_PASSWORD:
            ...
            uncomment(sshd_config,
                    '#(\s?)PasswordAuthentication(\s*)no',
                    use_sudo=True)
            sudo('/etc/init.d/ssh restart')
...
def set_timezone(rollback=False):
    ...
    """
    if not rollback:
        if contains(filename='/etc/timezone',
                text=env.TIME_ZONE,
                use_sudo=True):
            return False
        if env.verbosity:
            print env.host, "CHANGING TIMEZONE /etc/timezone to " + \
                env.TIME_ZONE
...
def setup_ufw_rules():
    ...
    if not env.overwrite and firewall_rules == current_rules:
        return
    if env.verbosity:
...
def upgrade_packages():
    ...
    if env.verbosity:
        ...
        print " NOTE: apt-get upgrade has been known in rare cases to " \
            "require user input."
        print "If apt-get upgrade does not complete within 15 minutes"
        print "see troubleshooting docs *before* aborting the process " \
            "to avoid package management corruption."
...
def upload_etc():
    ...
    if env.project_template_dir:
        user_templates = _get_template_files(
                os.path.join(env.project_template_dir, ''))
    else:
        user_templates = set([])
    ...
    for t in etc_templates:
        ...
        if directory in [
            ...
            continue
        uploaded = upload_template(t, dest, context=context,
                use_sudo=True,
                modified_only=mod_only)
...
def upload_ssh_key(rollback=False):
    ...
    if not rollback:
        ...
        if env.key_filename and env.INTERACTIVE:
            upload_key = confirm(
                    'Would you like to upload your personal key ' \
                        'in addition to %s' % str(env.key_filename),
                    default=True)
        if upload_key:
