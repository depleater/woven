...
woven_env = _AttributeDict({
    ...
    'HOSTS': [],
    # Optional as per fabric. eg {'staging': ['woven@example.com']}.
    'ROLEDEFS': {},
        ...
        if not vers.isdigit():
            for num, char in enumerate(vers):
                if char.isdigit():
                    ...
                    # We will strip spaces to avoid needing to 'quote'
                    # paths.
                    stage_sep = '-'
                    stage = vers[num + 1:]
                    break
...
def disable_virtualenvwrapper():
    """
    Hack to workaround an issue with virtualenvwrapper logging caused
    by Fabric sudo.
...
def post_install_package():
    """
    Run any functions post install a matching package. Hook functions
    are in the form post_install_[package name] and are defined in a
    deploy.py file.
...
def set_server_state(name, object=None, delete=False):
    """
    Sets a simple 'state' on the server by creating a file with the
    desired state's name and storing ``content`` as json strings if
    supplied.
...
def set_version_state(name, object=None, delete=False):
    """
    Sets a simple 'state' on the server by creating a file with the
    desired state's name + version and storing ``content`` as json
    strings if supplied.
