...
def activate():
    """
    Activates the version specified in ``env.project_version`` if it
    is different from the current active version.
    ...
    with cd('/'.join([
        ...
        for site in site_ids:
            for settings_file in _sitesettings_files():
                ...
                if env.verbosity:
                    print " * django-admin.py syncdb --noinput " \
                        "--settings=%s" % site_settings
                output = sudo(' '.join([
                            'source',
                            venv,
                            '&&',
...
def manual_migration():
    ...
    Option 1: Enter y to exit the current deployment. When migration
    is completed run deploy again.
    ...
    if env.INTERACTIVITY:
        ...
        print "Option 1: Enter y to exit the current deployment. " \
            "When migration is completed run deploy again."
        print "Option 2: run the migration in a separate shell"
        exit = confirm("Enter y to exit or accept default to " \
                    "complete deployment and activate the new version",
                default=False)
    else:
        exit = True
    if exit:
...
def rmvirtualenv():
    """
    Remove the current or ``env.project_version`` environment and all
    content in it.
    """
...
def pip_install_requirements():
    """
    Install on current installed virtualenv version from a pip bundle
    [dist/project name-version].zip or pip
    ``req.txt``|``requirements.txt`` or a env.pip_requirements list.

    By default it will look for a zip bundle in the dist directory
    first then a requirements file.

    The limitations of installing requirements are that you cannot
    point directly to packages in your local filesystem. In this case
    you would bundle instead.
    """
    if not version_state('mkvirtualenv'):
        print env.host, 'Error: Cannot run pip_install_requirements. ' \
            'A virtualenv is not created for this version. ' \
            'Run mkvirtualenv first'
        return
    if env.verbosity:
    ...
    if exists(pip_log_dir):
        run('rm -f %s/*.txt' % pip_log_dir)
    ...
    if svn_version > -1:
        ...
        for b in other_builds:
            if b in django_version:
                ...
                print "Define a DJANGO_REQUIREMENT pointing to " \
                    "the tar.gz for", django_version
                print "and re-deploy, or use the official or SVN release " \
                    "of Django."
                sys.exit(1)
    ...
    if req_files:
        file_patterns = '|'.join([file_patterns, 'req*.zip'])
    ...
    remote_dir = '/'.join([deployment_root(), 'env',
            env.project_fullname, 'dist'])
    if os.path.exists(local_dir):
        if file_patterns:
            deployed += deploy_files(local_dir,
                    remote_dir, pattern=file_patterns)
    ...
    python_path = '/'.join([deployment_root(), 'env',
            env.project_fullname, 'bin', 'python'])
    with settings(warn_only=True):
        with cd(remote_dir):
            for req in req_files_list:
                ...
                if '.zip' in req.lower():
                    install = run(
                            'pip install %s -q --environment=%s ' \
                                '--log=/home/%s/.pip/%s_pip_log.txt' % (
                                    req, python_path, env.user,
                                    req.replace('.', '_')))

                else:
                    install = run(
                            'pip install -q --environment=%s --src=%s ' \
                                '--download-cache=%s --requirement=%s ' \
                                '--log=/home/%s/.pip/%s_pip_log.txt' % (
                                        python_path, src, cache, req, env.user,
                                        req.replace('.', '_')))
                if install.failed:
                    out.failed = True
                    out.stderr += ' '.join([env.host,
    ...
    if out.failed:
        print out.stderr
        print "Review the pip install logs at %s/.pip and re-deploy" % \
            deployment_root()
        sys.exit(1)
    return out
