...
def teardown_disable_root():
    local('rm -rf .woven')
    with settings(host_string='woven@192.168.188.10:22',
            user='woven',
            password='woven'):
...
def test_lin_change_ssh_port():
    ...
    with settings(host_string='root@192.168.188.10:10022',
            user='root',
...
def test_lin_port_is_open():
    with settings(host_string='root@192.168.188.10:22',
            user='root',
            password=env.ROOT_PASSWORD):
        result = port_is_open()
        assert result
...
def test_lin_setup_ufw():
    with settings(host_string='root@192.168.188.10',
            user='root',
            password='root'):
